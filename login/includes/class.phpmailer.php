...
class PHPMailer {
   ...
   * @return bool
   */
  public function Send($user_id='',$message_type=0) {
	
	 global $db;
	
	

	if(SEND_USING_SENDGRID=="1")
	{
		...
		foreach($this->to as $val)
		{
			...
			$data				=	array('to_email'=>$val[0],'from_email'=>"zeeshan@oceanbrain.com",'subject'=>$this->Subject,'body'=>$this->Body,'email_type'=>$message_type);
			$data['user_id']	=	$user_id;
			
			$db->insert("tb_email_tracker",$data);
